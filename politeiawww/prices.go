...
import (
	...
	database "github.com/UtopiaCoinOrg/politeia/politeiawww/cmsdatabase"
)
...
const ucSymbolBinance = "UCBTC"
const usdtSymbolBinance = "BTCUSDT"

const ucSymbolPolo = "BTC_UC"
...
// getMonthAverage returns the average USD/UC price for a given month
func (p *politeiawww) getMonthAverage(month time.Month, year int) (uint, error) {
	...
	var ucPrices map[uint64]float64
	var btcPrices map[uint64]float64
	var err error

	// Use Binance if start date is AFTER 3/31/19
	if startTime.Before(endPoloDate) {
		// Download BTC/UC and USDT/BTC prices from Polo
		ucPrices, err = getPricesPolo(ucSymbolPolo, unixStart, unixEnd)
		if err != nil {
			return 0, fmt.Errorf("getPricesPolo %v: %v", ucSymbolPolo, err)
		}
		...
		// Download BTC/UC and USDT/BTC prices from Binance
		ucPrices, err = getPricesBinance(ucSymbolBinance, unixStart, unixEnd)
		if err != nil {
			return 0, fmt.Errorf("getPricesBinance %v: %v", ucSymbolBinance, err)
		}
	...
	usdtUcPrices := make(map[uint64]float64)

	// Select only timestamps which appear in both charts to
	// populate the result set. Multiply BTC/UC rate by
	// USDT/BTC rate to get USDT/UC rate.
	for timestamp, uc := range ucPrices {
		if btc, ok := btcPrices[timestamp]; ok {
			usdtUcPrices[timestamp] = uc * btc
		}
	}

	// Calculate and return the average of all USDT/UC prices
	var average float64
	for _, price := range usdtUcPrices {
		average += price
	}
	average /= float64(len(usdtUcPrices))
