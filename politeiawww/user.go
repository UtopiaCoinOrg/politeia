...
func (p *politeiawww) processUserCommentsLikes(user *user.User, token string) (*www.UserCommentsLikesReply, error) {
	...
	dlc, err := p.utopiaPropCommentLikes(token)
	if err != nil {
		return nil, fmt.Errorf("utopiaPropLikeComments: %v", err)
	}
	...
	for _, v := range dlc {
		...
		if user.ID.String() == u.ID.String() {
			lc = append(lc, convertLikeCommentFromUtopia(v))
		}
	}
...
https://github.com/UtopiaCoinOrg/politeia/issues/860#issuecomment-520871500
