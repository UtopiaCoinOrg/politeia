...
import (
	...
	pstypes "github.com/UtopiaCoinOrg/ucdata/pubsub/types/v2"
...
func (p *politeiawww) addWatchAddress(address string) {
	err := p.wsUcdata.subToAddr(address)
	if err != nil {
...
func (p *politeiawww) removeWatchAddress(address string) {
	err := p.wsUcdata.unsubFromAddr(address)
	if err != nil {
...
func (p *politeiawww) setupCMSAddressWatcher() {
	p.wsUcdata.subToPing()
	go func() {
		for {
			msg, ok := <-p.wsUcdata.client.Receive()
			if !ok {
				break
			}
			if msg == nil {
		...
		if err != nil {
			if err == database.ErrInvoiceNotFound {
				...
				invoice.Payments = database.Payments{
					...
					AmountNeeded: int64(payout.UCTotal),
				}
...
func (p *politeiawww) checkHistoricalPayments(payment *database.Payments) bool {
	...
	amountReceived := ucutil.Amount(0)
	...
	for i, tx := range txs {
		...
		amountReceived += ucutil.Amount(tx.Amount)
		if i == 0 {
			txIDs = tx.TxID
		} else {
			txIDs += ", " + tx.TxID
...
func (p *politeiawww) reconnectWS() {
	...
	var err error
	// Retry wsUcdata reconnect every 1 minute
	for {
		p.wsUcdata, err = newWSUcdata()
		if err != nil {
		...
		log.Infof("Retrying ws ucdata reconnect in 1 minute...")
