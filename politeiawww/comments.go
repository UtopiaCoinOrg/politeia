...
func (p *politeiawww) initCommentScores() error {
	...
	// Fetch UtopiaCoinOrg plugin inventory from cache
	ir, err := p.utopiaInventory()
	if err != nil {
		return fmt.Errorf("utopiaInventory: %v", err)
	}
...
func (p *politeiawww) getComment(token, commentID string) (*www.Comment, error) {
	...
	dc, err := p.utopiaCommentGetByID(token, commentID)
	if err != nil {
		return nil, fmt.Errorf("utopiaGetComment: %v", err)
	}
	c := convertCommentFromUtopia(*dc)
...
func (p *politeiawww) updateCommentScore(token, commentID string) (int64, error) {
	...
	likes, err := p.utopiaCommentLikes(token, commentID)
	if err != nil {
		return 0, fmt.Errorf("utopiaLikeComments: %v", err)
	}
...
// processNewComment sends a new comment UtopiaCoinOrg plugin command to politeaid
...
func (p *politeiawww) processNewComment(nc www.NewComment, u *user.User) (*www.NewCommentReply, error) {
	...
	vdr, err := p.utopiaVoteDetails(nc.Token)
	if err != nil {
		return nil, fmt.Errorf("utopiaVoteDetails: %v", err)
	}
	_, avr := convertAuthVoteFromUtopia(vdr.AuthorizeVote)
	svr := convertStartVoteReplyFromUtopia(vdr.StartVoteReply)
	...
	_, err = p.utopiaCommentGetBySignature(nc.Token, nc.Signature)
	if err != cache.ErrRecordNotFound {
		return nil, www.UserError{
	...
	dnc := convertNewCommentToUtopiaPlugin(nc)
	payload, err := ucplugin.EncodeNewComment(dnc)
	if err != nil {
		return nil, err
	}
	...
	pc := pd.PluginCommand{
		...
		ID:        ucplugin.ID,
		Command:   ucplugin.CmdNewComment,
		CommandID: ucplugin.CmdNewComment,
	...
	ncr, err := ucplugin.DecodeNewCommentReply([]byte(reply.Payload))
	if err != nil {
		return nil, err
	}
...
// processNewCommentInvoice sends a new comment UtopiaCoinOrg plugin command to politeaid
// then fetches the new comment from the cache and returns it.
...
func (p *politeiawww) processLikeComment(lc www.LikeComment, u *user.User) (*www.LikeCommentReply, error) {
	...
	// Ensure proposal voting has not ended
	vdr, err := p.utopiaVoteDetails(lc.Token)
	if err != nil {
		return nil, fmt.Errorf("utopiaVoteDetails: %v", err)
	}
	...
	_, err = p.utopiaCommentGetByID(lc.Token, lc.CommentID)
	if err != nil {
		if err == cache.ErrRecordNotFound {
			err = www.UserError{
	...
	dlc := convertLikeCommentToUtopia(lc)
	payload, err := ucplugin.EncodeLikeComment(dlc)
	if err != nil {
		return nil, err
	}
	...
	pc := pd.PluginCommand{
		Challenge: hex.EncodeToString(challenge),
		ID:        ucplugin.ID,
		Command:   ucplugin.CmdLikeComment,
		CommandID: ucplugin.CmdLikeComment,
		Payload:   string(payload),
	}
	...
	lcr, err := ucplugin.DecodeLikeCommentReply([]byte(reply.Payload))
	if err != nil {
		return nil, err
	}
...
// processCensorComment sends a censor comment UtopiaCoinOrg plugin command to
...
func (p *politeiawww) processCensorComment(cc www.CensorComment, u *user.User) (*www.CensorCommentReply, error) {
	...
	c, err := p.utopiaCommentGetByID(cc.Token, cc.CommentID)
	if err != nil {
		return nil, fmt.Errorf("utopiaGetComment: %v", err)
	}
	if c.Censored {
	...
	// Ensure proposal voting has not ended
	vdr, err := p.utopiaVoteDetails(cc.Token)
	if err != nil {
		return nil, fmt.Errorf("utopiaVoteDetails: %v", err)
	}
	vd := convertVoteDetailsReplyFromUtopia(*vdr)
	...
	dcc := convertCensorCommentToUtopia(cc)
	payload, err := ucplugin.EncodeCensorComment(dcc)
	if err != nil {
		return nil, err
	}
	...
	pc := pd.PluginCommand{
		Challenge: hex.EncodeToString(challenge),
		ID:        ucplugin.ID,
		Command:   ucplugin.CmdCensorComment,
		CommandID: ucplugin.CmdCensorComment,
		Payload:   string(payload),
	}
	...
	ccr, err := ucplugin.DecodeCensorCommentReply([]byte(reply.Payload))
	if err != nil {
		return nil, err
	}
