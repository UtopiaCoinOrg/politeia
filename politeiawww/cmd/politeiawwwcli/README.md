...
`https://proposals.UtopiaCoinOrg.org`.  Copy these lines into your
...
`--paywall` will use the UtopiaCoinOrg testnet faucet to satisfy the user registration
fee requirement. 
...
prevention measure that would normally need to be purchased using UC, but if
...
$ politeiawww_dbutil -testnet -aducedits username 50
```
...
[politeiavoter](https://github.com/UtopiaCoinOrg/politeia/tree/master/politeiavoter/)
tool.

### Voting on a proposal - politeiawwwcli
You can also vote on proposals using `politeiawwwcli`, but for right now, it 
only works on testnet and you have to be running your ucwallet locally using
