...
func (cmd *TestRunCmd) Execute(args []string) error {
	...
	if paywallEnabled {
		...
		uc := float64(lr.PaywallAmount) / 1e8
		fmt.Printf("  Paid %v UC to %v with txID %v\n",
			uc, lr.PaywallAddress, txID)
	}
	...
	avc.Args.Action = ucplugin.AuthVoteActionAuthorize
	err = avc.Execute(nil)
	if err != nil {
		return err
	}
	...
	avc.Args.Action = ucplugin.AuthVoteActionRevoke
	err = avc.Execute(nil)
	if err != nil {
		return err
	}
	...
	if err != nil {
		...
		case strings.Contains(err.Error(), "connection refused"):
			// User is not running a ucwallet instance locally.
			// This is ok. Print a warning and continue.
			fmt.Printf("  WARNING: could not connect to ucwallet; " +
				"skipping cast votes\n")
			skipCastVotes = true
...
Paywall: 
If the politeiawww paywall is enabled the test run will use the UtopiaCoinOrg tesnet
...
Voting:
The test run will attempt to vote on a proposal.  If a ucwallet instance is
