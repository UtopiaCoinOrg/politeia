...
import (
	...
	cms "github.com/UtopiaCoinOrg/politeia/politeiawww/api/cms/v1"
	www "github.com/UtopiaCoinOrg/politeia/politeiawww/api/www/v1"
	"github.com/UtopiaCoinOrg/politeia/politeiawww/cmsdatabase"
)

func convertCastVoteReplyFromUtopiaPlugin(cvr ucplugin.CastVoteReply) www.CastVoteReply {
...
func convertCastVoteFromWWW(b www.CastVote) ucplugin.CastVote {
...
func convertBallotFromWWW(b www.Ballot) ucplugin.Ballot {
	br := ucplugin.Ballot{
		Votes: make([]ucplugin.CastVote, 0, len(b.Votes)),
	}
...
func convertBallotReplyFromUtopiaPlugin(b ucplugin.BallotReply) www.BallotReply {
	br := www.BallotReply{
		Receipts: make([]www.CastVoteReply, 0, len(b.Receipts)),
	}
	for _, v := range b.Receipts {
		br.Receipts = append(br.Receipts,
			convertCastVoteReplyFromUtopiaPlugin(v))
	}
	return br
}

func convertVoteOptionFromWWW(vo www.VoteOption) ucplugin.VoteOption {
	return ucplugin.VoteOption{
		Id:          vo.Id,
...
func convertVoteOptionsFromWWW(vo []www.VoteOption) []ucplugin.VoteOption {
	vor := make([]ucplugin.VoteOption, 0, len(vo))
	for _, v := range vo {
...
func convertVoteFromWWW(v www.Vote) ucplugin.Vote {
	return ucplugin.Vote{
		Token:            v.Token,
		Mask:             v.Mask,
		Duration:         v.Duration,
...
func convertAuthorizeVoteFromWWW(av www.AuthorizeVote) ucplugin.AuthorizeVote {
...
func convertStartVoteFromWWW(sv www.StartVote) ucplugin.StartVote {
	return ucplugin.StartVote{
...
func convertNewCommentToUtopiaPlugin(nc www.NewComment) ucplugin.NewComment {
	return ucplugin.NewComment{
		Token:     nc.Token,
...
func convertLikeCommentToUtopia(lc www.LikeComment) ucplugin.LikeComment {
...
func convertLikeCommentFromUtopia(lc ucplugin.LikeComment) www.LikeComment {
...
func convertCensorCommentToUtopia(cc www.CensorComment) ucplugin.CensorComment {
	return ucplugin.CensorComment{
		Token:     cc.Token,
...
func convertCommentFromUtopia(c ucplugin.Comment) www.Comment {
...
func convertAuthVoteFromUtopia(dav ucplugin.AuthorizeVote) (www.AuthorizeVote, www.AuthorizeVoteReply) {
...
func convertStartVoteFromUtopia(sv ucplugin.StartVote) www.StartVote {
...
func convertStartVoteReplyFromUtopia(svr ucplugin.StartVoteReply) www.StartVoteReply {
...
func convertVoteDetailsReplyFromUtopia(vdr ucplugin.VoteDetailsReply) VoteDetails {
	av, avr := convertAuthVoteFromUtopia(vdr.AuthorizeVote)
	return VoteDetails{
		AuthorizeVote:      av,
...
func convertCastVoteFromUtopia(cv ucplugin.CastVote) www.CastVote {
	return www.CastVote{
		Token:     cv.Token,
		Ticket:    cv.Ticket,
		VoteBit:   cv.VoteBit,
...
func convertCastVotesFromUtopia(cv []ucplugin.CastVote) []www.CastVote {
	cvr := make([]www.CastVote, 0, len(cv))
	for _, v := range cv {
		cvr = append(cvr, convertCastVoteFromUtopia(v))
	}
	return cvr
}

func convertVoteResultsReplyFromUtopia(vrr ucplugin.VoteResultsReply) (www.StartVote, []www.CastVote) {
	sv := convertStartVoteFromUtopia(vrr.StartVote)
	cv := convertCastVotesFromUtopia(vrr.CastVotes)
	return sv, cv
}
...
func convertVoteOptionResultsFromUtopia(vor []ucplugin.VoteOptionResult) []www.VoteOptionResult {
...
func convertTokenInventoryReplyFromUtopia(r ucplugin.TokenInventoryReply) www.TokenInventoryReply {
