...
import (
	...
	www "github.com/UtopiaCoinOrg/politeia/politeiawww/api/www/v1"
...
const (
	...
	// Note that 14 is in use by the UtopiaCoinOrg plugin
	// Note that 15 is in use by the UtopiaCoinOrg plugin
...
func voteIsAuthorized(avr www.AuthorizeVoteReply) bool {
	if avr.Receipt == "" {
		...
		return false
	} else if avr.Action == ucplugin.AuthVoteActionRevoke {
...
func voteStatusFromVoteSummary(r ucplugin.VoteSummaryReply, bestBlock uint64) www.PropVoteStatusT {
	switch {
	case !r.Authorized:
		return www.PropVoteStatusNotAuthorized
	case r.EndHeight == "":
...
func (p *politeiawww) getProp(token string) (*www.ProposalRecord, error) {
	...
	dc, err := p.utopiaGetComments(token)
	if err != nil {
		log.Errorf("getProp: utopiaGetComments failed "+
			"for token %v", token)
	}
...
func (p *politeiawww) getProps(tokens []string) (*[]www.ProposalRecord, error) {
	...
	dnc, err := p.utopiaGetNumComments(tokens)
	if err != nil {
		log.Errorf("getProp: utopiaGetNumComments failed "+
			"for tokens %v", tokens)
	} else {
...
func (p *politeiawww) getPropVersion(token, version string) (*www.ProposalRecord, error) {
	...
	dc, err := p.utopiaGetComments(token)
	if err != nil {
		log.Errorf("getPropVersion: utopiaGetComments "+
...
func (p *politeiawww) getAllProps() ([]www.ProposalRecord, error) {
	...
	for _, v := range records {
		...
		dc, err := p.utopiaGetComments(pr.CensorshipRecord.Token)
		if err != nil {
			log.Errorf("getAllProps: utopiaGetComments failed "+
...
func (p *politeiawww) getPropComments(token string) ([]www.Comment, error) {
	...
	dc, err := p.utopiaGetComments(token)
	if err != nil {
		return nil, fmt.Errorf("utopiaGetComments: %v", err)
	}
...
func (p *politeiawww) processSetProposalStatus(sps www.SetProposalStatus, u *user.User) (*www.SetProposalStatusReply, error) {
	...
	case pr.State == www.PropStateVetted:
		...
		vdr, err := p.utopiaVoteDetails(pr.CensorshipRecord.Token)
		if err != nil {
			return nil, fmt.Errorf("utopiaVoteDetails: %v", err)
		}
		vd := convertVoteDetailsReplyFromUtopia(*vdr)
...
func (p *politeiawww) processEditProposal(ep www.EditProposal, u *user.User) (*www.EditProposalReply, error) {
	...
	vdr, err := p.utopiaVoteDetails(ep.Token)
	if err != nil {
		return nil, fmt.Errorf("utopiaVoteDetails: %v", err)
	}
	vd := convertVoteDetailsReplyFromUtopia(*vdr)
...
func (p *politeiawww) voteStatusReply(token string, bestBlock uint64) (*www.VoteStatusReply, error) {
	...
	r, err := p.utopiaVoteSummary(token)
	if err != nil {
		return nil, err
	}

	results := convertVoteOptionResultsFromUtopia(r.Results)
	var total uint64
...
func (p *politeiawww) processActiveVote() (*www.ActiveVoteReply, error) {
	...
	for _, v := range all {
		...
		vdr, err := p.utopiaVoteDetails(v.CensorshipRecord.Token)
		if err != nil {
			log.Errorf("processActiveVote: utopiaVoteDetails failed %v: %v",
...
func (p *politeiawww) processVoteResults(token string) (*www.VoteResultsReply, error) {
	...
	// Get vote details from cache
	vdr, err := p.utopiaVoteDetails(token)
	if err != nil {
		return nil, fmt.Errorf("utopiaVoteDetails: %v", err)
	}

	// Get cast votes from cache
	vrr, err := p.utopiaProposalVotes(token)
	if err != nil {
		return nil, fmt.Errorf("utopiaVoteDetails: %v", err)
	}

	return &www.VoteResultsReply{
		StartVote:      convertStartVoteFromUtopia(vdr.StartVote),
		StartVoteReply: convertStartVoteReplyFromUtopia(vdr.StartVoteReply),
		CastVotes:      convertCastVotesFromUtopia(vrr.CastVotes),
	}, nil
}
...
func (p *politeiawww) processCastVotes(ballot *www.Ballot) (*www.BallotReply, error) {
	...
	payload, err := ucplugin.EncodeBallot(convertBallotFromWWW(*ballot))
	if err != nil {
		return nil, err
	}
	pc := pd.PluginCommand{
		Challenge: hex.EncodeToString(challenge),
		ID:        ucplugin.ID,
		Command:   ucplugin.CmdBallot,
		CommandID: ucplugin.CmdBallot,
	...
	// Decode plugin reply
	br, err := ucplugin.DecodeBallotReply([]byte(reply.Payload))
	if err != nil {
		return nil, err
	}
	brr := convertBallotReplyFromUtopiaPlugin(*br)
	return &brr, nil
}
...
// processAuthorizeVote sends the authorizevote command to UtopiaCoinOrg plugin to
...
func (p *politeiawww) processAuthorizeVote(av www.AuthorizeVote, u *user.User) (*www.AuthorizeVoteReply, error) {
	...
	// Get vote details from cache
	vdr, err := p.utopiaVoteDetails(av.Token)
	if err != nil {
		return nil, fmt.Errorf("utopiaVoteDetails: %v", err)
	}
	vd := convertVoteDetailsReplyFromUtopia(*vdr)
	...
	case av.Action != ucplugin.AuthVoteActionAuthorize &&
		av.Action != ucplugin.AuthVoteActionRevoke:
	...
	case av.Action == ucplugin.AuthVoteActionRevoke &&
		!voteIsAuthorized(vd.AuthorizeVoteReply):
	...
	payload, err := ucplugin.EncodeAuthorizeVote(dav)
	if err != nil {
	...
	pc := pd.PluginCommand{
		Challenge: hex.EncodeToString(challenge),
		ID:        ucplugin.ID,
		Command:   ucplugin.CmdAuthorizeVote,
		CommandID: ucplugin.CmdAuthorizeVote + " " + av.Token,
		Payload:   string(payload),
	}
	...
	// Decode plugin reply
	avr, err := ucplugin.DecodeAuthorizeVoteReply([]byte(reply.Payload))
	if err != nil {
		return nil, fmt.Errorf("DecodeAuthorizeVoteReply: %v", err)
	}

	if !p.test && avr.Action == ucplugin.AuthVoteActionAuthorize {
...
func (p *politeiawww) processStartVote(sv www.StartVote, u *user.User) (*www.StartVoteReply, error) {
	...
	payload, err := ucplugin.EncodeStartVote(dsv)
	if err != nil {
		return nil, err
	}
	...
	// Get vote details from cache
	vdr, err := p.utopiaVoteDetails(sv.Vote.Token)
	if err != nil {
		return nil, fmt.Errorf("utopiaVoteDetails: %v", err)
	}
	vd := convertVoteDetailsReplyFromUtopia(*vdr)
	...
	// Tell UtopiaCoinOrg plugin to start voting
	challenge, err := util.Random(pd.ChallengeSize)
	if err != nil {
		return nil, err
	}
	...
	pc := pd.PluginCommand{
		Challenge: hex.EncodeToString(challenge),
		ID:        ucplugin.ID,
		Command:   ucplugin.CmdStartVote,
		CommandID: ucplugin.CmdStartVote + " " + sv.Vote.Token,
		Payload:   string(payload),
	}
	...
	vr, err := ucplugin.DecodeStartVoteReply([]byte(reply.Payload))
	if err != nil {
		return nil, err
	}
	...
	// return a copy
	rv := convertStartVoteReplyFromUtopia(*vr)
	return &rv, nil
}
...
func (p *politeiawww) processTokenInventory(isAdmin bool) (*www.TokenInventoryReply, error) {
	...
	for retries := 0; !done && retries <= 1; retries++ {
		...
		// for non-admins.
		ti, err := p.utopiaTokenInventory(bb, isAdmin)
		if err != nil {
			if err == cache.ErrRecordNotFound {
				// There are missing entries in the vote
				// results cache table. Load them.
				_, err := p.utopiaLoadVoteResults(bb)
				if err != nil {
					return nil, err
				}
		...
		r = convertTokenInventoryReplyFromUtopia(*ti)
		done = true
	}
