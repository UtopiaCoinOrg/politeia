...
func (p *politeiawww) validateInvoice(ni cms.NewInvoice, u *user.User) error {
	...
	for _, v := range ni.Files {
		...
		if strings.HasPrefix(v.MIME, "image/") {
			...
			addr, err := ucutil.DecodeAddress(strings.TrimSpace(invInput.PaymentAddress))
			if err != nil {
				return www.UserError{
					ErrorCode: cms.ErrorStatusInvalidPaymentAddress,
				}
...
func (p *politeiawww) calculatePayout(inv database.Invoice) (cms.Payout, error) {
	...
	if inv.ExchangeRate > 0 {
		payout.UCTotal, err = ucutil.NewAmount(float64(payout.Total) /
