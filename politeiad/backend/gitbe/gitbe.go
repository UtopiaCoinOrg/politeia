// Copyright (c) 2017-2018 The UtopiaCoinOrg developers
...
import (
	...
	v1 "github.com/UtopiaCoinOrg/uctime/api/v1"
	"github.com/UtopiaCoinOrg/uctime/merkle"
	...
	"github.com/UtopiaCoinOrg/politeia/politeiad/api/v1/mime"
...
var (
	...
	defaultRepoConfig = map[string]string{
		...
		"core.autocrlf": "false",
		"user.name":     "Politeia",
		"user.email":    "noreply@UtopiaCoinOrg.org",
	}
...
type gitBackEnd struct {
	...
	uctimeHost     string           // Uctimed host
...
// anchor takes a slice of commit digests and anchors them in uctime.
//
// This function is being clever with the anchors.  It sends two values to
// uctime.  We anchor the merkle root, and we *also* anchor all
...
// truly curious.  This is essentially free because uctime compresses all
// digests into a single merkle root.
//
// This function should be called with the lock held.
// TODO: the physical write to uctime needs to come out of the lock.
func (g *gitBackEnd) anchor(digests []*[sha256.Size]byte) error {
	...
	return util.Timestamp("politeia", g.uctimeHost, digests)
}
...
func (g *gitBackEnd) anchorRepo(path string) (*[sha256.Size]byte, error) {
	...
	// politeia's lookup key but uctime will likely return a different
	// merkle.  Uctime returns a different merkle when there are
...
// case it drops and anchor in uctime for each of them.
...
func (g *gitBackEnd) afterAnchorVerify(vrs []v1.VerifyDigest) error {
	...
	for _, vr := range vrs {
		if vr.ChainInformation.ChainTimestamp == 0 {
			// uctime returns 0 when there are not enough
			// confirmations yet.
		...
		// Mark test anchors as confirmed by uctime
		if g.test {
...
// verifyAnchor asks uctime if an anchor has been verified and returns a TX if
// it has.
func (g *gitBackEnd) verifyAnchor(digest string) (*v1.VerifyDigest, error) {
	...
	if g.test {
		...
		// Call uctime
		vr, err = util.Verify("politeia", g.uctimeHost,
			[]string{digest})
		if err != nil {
			return nil, err
		}
...
func (g *gitBackEnd) _updateRecord(commit bool, id string, mdAppend, mdOverwrite []backend.MetadataStream, fa []file, filesDel []string) error {
	...
	avFilename := fmt.Sprintf("%02v%v", ucplugin.MDStreamAuthorizeVote,
		defaultMDFilenameSuffix)
	...
	// Call plugin hooks
	f, ok := utopiaPluginHooks[PluginPostHookEdit]
	if ok {
...
// uctime.  This is an expensive operation and should not be run during
// runtime.
//
...
func (g *gitBackEnd) fsck(path string) error {
	...
	var seenAnchor bool
	// gitDigests is an index of all git digests to verify with uctime
	gitDigests := make(map[string]struct{})
	// confirmedAnchors keeps track of anchors that were timestamped with uctime but not verified,
	...
	for _, v := range out {
		if regexAnchorConfirmation.MatchString(v) {
			...
			// If we didn't, save the anchor key to verify with uctime later
	...
	log.Infof("fsck: uctime verification started")
	...
	vr, err := util.Verify("politeia", g.uctimeHost, digests)
	if err != nil {
		return err
	}
	...
	for _, v := range vr.Digests {
		if v.Result != v1.ResultOK {
			fail = true
			log.Errorf("uctime error: %v %v %v", v.Digest,
				v.Result, v1.Result[v.Result])
		}
	}
	if fail {
		return fmt.Errorf("uctime fsck failed")
	}
...
func New(anp *chaincfg.Params, root string, uctimeHost string, gitPath string, id *identity.FullIdentity, gitTrace bool) (*gitBackEnd, error) {
	...
	g := &gitBackEnd{
		...
		gitPath:         gitPath,
		uctimeHost:     uctimeHost,
		gitTrace:        gitTrace,
		exit:            make(chan struct{}),
		checkAnchor:     make(chan struct{}),
		testAnchors:     make(map[string]bool),
		plugins:         []backend.Plugin{getUtopiaPlugin(anp.Name != "mainnet")},
	}
	idJSON, err := id.Marshal()
	if err != nil {
		return nil, err
	}
	setUtopiaPluginSetting(utopiaPluginIdentity, string(idJSON))
	setUtopiaPluginSetting(utopiaPluginJournals, g.journals)
	setUtopiaPluginHook(PluginPostHookEdit, g.utopiaPluginPostEdit)
	...
	err = g.initUtopiaPluginJournals()
	if err != nil {
		return nil, err
	}
	...
	// Message user
	log.Infof("Timestamp host: %v", g.uctimeHost)

	log.Infof("Running uctime fsck on vetted repository")
	err = g.fsck(g.vetted)
	if err != nil {
		// Log error but continue
		log.Errorf("fsck: uctime %v", err)
	}
