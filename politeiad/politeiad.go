...
import (
	...
	"github.com/UtopiaCoinOrg/politeia/politeiad/api/v1"
	"github.com/UtopiaCoinOrg/politeia/politeiad/api/v1/identity"
	"github.com/UtopiaCoinOrg/politeia/politeiad/backend"
	"github.com/UtopiaCoinOrg/politeia/politeiad/backend/gitbe"
	"github.com/UtopiaCoinOrg/politeia/politeiad/cache"
	"github.com/UtopiaCoinOrg/politeia/politeiad/cache/cachestub"
	"github.com/UtopiaCoinOrg/politeia/politeiad/cache/cockroachdb"
	"github.com/UtopiaCoinOrg/politeia/util"
	"github.com/UtopiaCoinOrg/politeia/util/version"
...
func _main() error {
	...
	if len(loadedCfg.UctimeCert) != 0 {
		var certPool *x509.CertPool
		if !util.FileExists(loadedCfg.UctimeCert) {
			return fmt.Errorf("unable to find uctime cert %v",
				loadedCfg.UctimeCert)
		}
		uctimeCert, err := ioutil.ReadFile(loadedCfg.UctimeCert)
		if err != nil {
			return fmt.Errorf("unable to read uctime cert %v: %v",
				loadedCfg.UctimeCert, err)
		}
		certPool = x509.NewCertPool()
		if !certPool.AppendCertsFromPEM(uctimeCert) {
	...
	b, err := gitbe.New(activeNetParams.Params, loadedCfg.DataDir,
		loadedCfg.UctimeHost, "", p.identity, loadedCfg.GitTrace)
	if err != nil {
		return err
	}
	p.backend = b
