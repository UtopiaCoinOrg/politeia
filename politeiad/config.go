...
// Copyright (c) 2015-2017 The UtopiaCoinOrg developers
...
import (
	...
	"github.com/UtopiaCoinOrg/uctime/api/v1"
	"github.com/UtopiaCoinOrg/politeia/politeiad/sharedconfig"
...
type config struct {
	...
	UctimeHost   string `long:"uctimehost" description:"Uctime ip:port"`
	UctimeCert   string `long:"uctimecert" description:"File containing the https certificate file for uctimehost"`
...
func loadConfig() (*config, []string, error) {
	...
	if cfg.TestNet {
		var timeHost string
		if len(cfg.UctimeHost) == 0 {
			timeHost = v1.DefaultTestnetTimeHost
		} else {
			timeHost = cfg.UctimeHost
		}
		cfg.UctimeHost = util.NormalizeAddress(timeHost,
	...
	cfg.UctimeHost = "https://" + cfg.UctimeHost

	if len(cfg.UctimeCert) != 0 && !util.FileExists(cfg.UctimeCert) {
		cfg.UctimeCert = cleanAndExpandPath(cfg.UctimeCert)
		path := filepath.Join(cfg.HomeDir, cfg.UctimeCert)
		if !util.FileExists(path) {
			str := "%s: uctimecert " + cfg.UctimeCert + " and " +
				path + " don't exist"
			err := fmt.Errorf(str, funcName)
			fmt.Fprintln(os.Stderr, err)
			return nil, nil, err
